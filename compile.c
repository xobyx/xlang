	...
	for (c = in; c != NULL; c = c->next)
	{
		...
		case itype:
		{
			...
			else //var deftion
			{
				...
				if (c_function != NULL)
				{
					define_new_var_on_function(name->value_char_ptr, c_function, c->value_type, &n_var);
				}
				else
				{
				...
				if (c->next->type_==s_index && c->next->opt_name_type == psize)
				{
